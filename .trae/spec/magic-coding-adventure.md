...
- **模块化设计**：
  - **计算模块**：负责核心业务逻辑和数据处理
    - `engine.ts`：游戏核心逻辑，包括战斗管理、状态更新
    - `calculator.ts`：法术计算逻辑，负责计算法术属性
    - `atb.ts`：ATB 系统和施法系统，处理战斗节奏
  - **UI 模块**：负责界面渲染和用户交互
    - `ui.ts`：集中管理所有显示逻辑，只负责渲染和事件响应
  - **存储模块**：负责数据持久化
    - `storage.ts`：处理游戏存档和读档
- **前端显示逻辑与计算逻辑分离**：
  - 计算逻辑封装在独立模块中，专注于数据处理
  - 显示逻辑集中在 UI 模块中，只负责界面渲染
  - 模块间通信优先使用事件系统，避免直接引用
  - 状态修改在计算模块中进行，通过事件通知 UI 模块更新
